...
module.exports = function (options) {
    ...
    function buildSearchPartial(requestQuery) {
        var searches = [];
        var colSearches = buildSearchArray(requestQuery);
        var globalSearches = buildSearchArray(requestQuery, true);
    ...
    function buildSearchArray(requestQuery, global) {
        ...
        _u.each(customColumns ? self.aSearchColumns : requestQuery.columns, function (column) {
            if (customColumns || column.searchable === 'true') {
                var colName = customColumns ? column : column.name,
                    searchVal = sanitize(global ? requestQuery.search.value : column.search.value);

                if (colName && searchVal) {
                    searchArray.push(buildILIKESearch(colName, searchVal));
                }
            }
        });
    ...
    function buildLimitPartial(requestQuery) {
        ...
        if (requestQuery && requestQuery.start !== undefined) {
            ...
            if (start >= 0) {
                ...
                sLimit = " OFFSET " + String(start) + " LIMIT ";
    ...
    function parseResponse(queryResult) {
        ...
        if (_u.isObject(queryResult) && _u.keys(queryResult).length > 1) {
            //console.log(`Extracted value from records total is: ${util.inspect(queryResult,{depth:null})}`);
    ...
    function extractResponseVal(res) {
        ...
        if(_u.isObject(res) && _u.isArray(res.rows) && res.rows.length){
            let values = _u.values(res.rows[0]);
            if(values.length){
